    ...
    <div className={`fixed bottom-8 left-8 z-50 transition-all duration-500 ${isExpanded ? 'w-80' : 'w-48'}`}>
       <div className="bg-slate-900 rounded-xl shadow-2xl border border-white/10 overflow-hidden ring-8 ring-black/10">
          <div className="relative aspect-video bg-slate-800">
             ...
             {/* AI Overlay Mockup */}
             <div className="absolute inset-0 pointer-events-none border-2 border-brand-orange/20 animate-pulse" />
             <div className="absolute top-2 left-2 flex items-center gap-1.5 px-2 py-1 bg-red-500 rounded-full text-[6px] font-black text-white uppercase tracking-widest z-10">
                   ...
                   <div className="space-y-2 max-h-32 overflow-y-auto pr-2 scrollbar-hide">
                      <AnimatePresence initial={false}>
                         {events.map((e) => (
                            ...
                            >
                               {e.status === 'warning' ? <AlertTriangle size={10} className="text-amber-400 mt-0.5" /> : e.status === 'success' ? <CheckCircle2 size={10} className="text-emerald-400 mt-0.5" /> : <Activity size={10} className="text-blue-400 mt-0.5" />}
