...
export default function AICallInterface({ onEndCall, userName }: AICallInterfaceProps) {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[100] bg-slate-950 flex flex-col items-center justify-center p-4 sm:p-8"
    >
      {/* Background Glow */}
           ...
           <AnimatePresence mode="wait">
              {status === 'connecting' ? (
                ...
                >
                  ...
                  <div className="text-brand-orange font-black uppercase tracking-[0.2em] text-sm animate-pulse">Establishing Secure Neural Link...</div>
                </motion.div>
              ) : (
                <motion.div 
                  key="active"
                      ...
                      <div className="flex items-center gap-1 h-32">
                        {[...Array(20)].map((_, i) => (
                          <motion.div
                            ...
                            className="w-1.5 bg-brand-orange rounded-full shadow-[0_0_15px_rgba(255,107,0,0.5)]"
                          />
                        ))}
                      </div>
                   </div>
