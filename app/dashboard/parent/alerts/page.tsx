...
<<<<<<< HEAD
            <div key={alert.id} className={`group p-8 rounded-xl border-2 transition-all ${
              alert.read 
                ...
                : 'border-orange-200 bg-gradient-to-r from-orange-50/50 to-slate-50 shadow-sm border-opacity-100 shadow-orange-200/50'
            }`}>
              <div className="flex items-start gap-6">
                <div className={`w-3 h-3 rounded-full mt-2 flex-shrink-0 transition-all ${
                  alert.read ? 'bg-slate-300' : 'bg-orange-500 animate-pulse'
                }`} />
                
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                      alert.type === 'Urgent' ? 'bg-red-100 text-red-700 border border-red-200' :
