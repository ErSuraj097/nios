      ...
      <div className="space-y-12 animate-fade-in pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           <div className="lg:col-span-2 p-12 rounded-[4rem] bg-white border border-slate-100 shadow-sm relative overflow-hidden group">
              ...
              <div className="flex flex-col md:flex-row items-center gap-10">
                 <div className="w-20 h-20 rounded-[2.5rem] bg-orange-50 text-brand-orange flex items-center justify-center shrink-0 shadow-sm">
                    <CreditCard size={32} />
                 </div>
                 <div className="text-center md:text-left">
                    <h3 className="text-2xl font-black text-slate-900 mb-2 uppercase tracking-tight">Financial Health</h3>
                    <p className="text-slate-500 font-medium leading-relaxed max-w-md">
                       All main examination fees for the 2026 academic cycle are up to date. <span className="text-brand-orange font-bold">1 pending submission fee</span> requires attention.
                    </p>
                 </div>
                 <div className="flex-1" />
                 <button className="px-10 py-5 bg-slate-900 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200">
                    Pay All Dues
                 </button>
              </div>
           </div>
           ...
           <div className="p-10 rounded-[4rem] bg-slate-900 text-white flex flex-col justify-between group overflow-hidden relative">
              ...
              <div className="flex items-center justify-between mb-8">
                 ...
                 <div className="text-[10px] font-black text-brand-orange uppercase tracking-widest bg-brand-orange/10 px-3 py-1 rounded-full border border-brand-orange/20">PCI-DSS Compliant</div>
              </div>
        ...
        <div className="space-y-8">
           <div className="flex flex-col md:flex-row gap-6 items-center justify-between px-4">
              ...
              <div className="flex gap-3">
                 <button className="px-6 py-4 bg-white border border-slate-100 rounded-3xl text-slate-400 font-black text-[10px] uppercase tracking-widest hover:text-slate-900 transition-colors flex items-center gap-3">
           ...
           <div className="grid gap-4">
              {fees.map((f) => (
                <div key={f.id} className="group p-8 rounded-[3rem] bg-white border border-slate-100 hover:border-brand-orange/20 hover:shadow-2xl hover:shadow-slate-200/50 transition-all flex flex-col lg:flex-row items-center gap-10">
                   <div className="w-16 h-16 rounded-[2rem] bg-slate-50 text-slate-400 flex items-center justify-center shrink-0 group-hover:bg-brand-orange group-hover:text-white transition-all shadow-sm">
                      <FileText size={24} />
                   </div>
                   
                   <div className="flex-1 text-center lg:text-left min-w-0">
                      <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">{f.child} · ID: {f.id}</div>
                      <h4 className="text-lg font-black text-slate-900 group-hover:text-brand-orange transition-colors truncate">{f.title}</h4>
                      <div className="flex items-center justify-center lg:justify-start gap-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                         <Calendar size={12} /> {f.date} · <Zap size={12} className="text-brand-orange" /> ONLINE REGISTRY
                      </div>
                   </div>
                   ...
                   <div className="flex items-center gap-6">
                      <div className="text-center min-w-[100px] hidden lg:block">
                         ...
                         <div className={`text-[10px] font-black uppercase tracking-widest ${
                           f.status === 'Paid' ? 'text-emerald-500' : 'text-brand-orange underline underline-offset-4 decoration-2'
                         }`}>{f.status}</div>
                      </div>
                      <button className={`px-10 py-5 rounded-[2rem] font-black text-[10px] uppercase tracking-widest transition-all shadow-xl active:scale-95 flex items-center gap-3 ${
                        f.status === 'Paid' ? 'bg-slate-50 text-slate-400' : 'bg-slate-900 text-white hover:bg-brand-orange'
                      }`}>
        ...
        <div className="p-10 rounded-[3rem] bg-white border border-slate-100 shadow-sm relative overflow-hidden group">
           ...
           <div className="flex flex-col lg:flex-row items-center gap-10">
              <div className="w-16 h-16 rounded-[2.5rem] bg-gradient-to-br from-orange-400 to-red-500 flex items-center justify-center shrink-0 shadow-xl shadow-orange-500/20">
              ...
              <button className="px-10 py-5 bg-slate-50 text-slate-900 rounded-[2rem] border border-slate-100 font-black text-[10px] uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all shadow-xl shadow-slate-200/50">
                 Manage Wallet
              </button>
           </div>
        </div>
