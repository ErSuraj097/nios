    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <button
          ...
          className={`p-3 text-sm font-black rounded-xl transition-all hover:shadow-md hover:scale-105 focus:outline-none focus:ring-2 focus:ring-brand-orange/50 relative group ${
            isSelected 
              ? 'bg-brand-orange text-white shadow-lg shadow-orange-500/30' 
              : isToday 
              ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30' 
              : dayEvents.length > 0 
              ? 'text-brand-orange border-2 border-brand-orange/30 hover:border-brand-orange' 
    ...
    >
              ...
              <div className="space-y-4 mb-8">
                {eventsOnDate(selectedDate).length > 0 ? (
                  eventsOnDate(selectedDate).map((event) => (
                    <div key={event.id} className="group p-6 bg-slate-50 rounded-xl border border-slate-200 hover:border-brand-orange/50 transition-all">
                      <div className="flex items-start gap-4 mb-3">
                        <div className="w-12 h-12 rounded-xl bg-brand-orange/10 border-2 border-brand-orange/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                          <Clock className="w-6 h-6 text-brand-orange" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <h4 className="font-black text-slate-900 text-lg group-hover:text-brand-orange truncate">{event.event || event.title}</h4>
                      ...
                      {event.status && (
                        <span className={`inline-block mt-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                          event.status === 'Live Now' ? 'bg-red-100 text-red-700' : 
                      ...
                      {event.joinLink && event.joinLink !== '#' && (
                        <button
                          ...
                          className="w-full mt-4 py-3 bg-brand-orange text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-orange-600 transition-all shadow-xl"
                        >
                          Join Session Now
                        </button>
                      )}
              ...
              <form onSubmit={handleAddEvent} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-4 border border-slate-200 rounded-xl font-black text-lg focus:ring-2 focus:ring-brand-orange focus:border-transparent transition-all"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-4 border border-slate-200 rounded-xl font-black focus:ring-2 focus:ring-brand-orange focus:border-transparent transition-all"
                  />
                </div>
                <div>
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-8 items-start animate-fade-in pb-20">
        ...
        <div className="space-y-10">
           {/* Featured Live Now */}
           <div className="p-12 rounded-xl bg-amber-50 border border-slate-200 shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative space-y-6">
                 ...
                 <h2 className="text-4xl lg:text-5xl font-black text-orange-800 tracking-tighter max-w-2xl leading-none">
                    Science: Advanced NewtonsLaws of Motion
                 </h2>

                 <div className="flex flex-wrap items-center gap-8 pt-4">
                    <div className="flex items-center gap-3">
                       <div className="w-12 h-12 rounded-xl bg-orange-500/5 border border-orange-500/10 flex items-center justify-center font-black text-orange-800">V</div>
                       <div className="text-left">
                          <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Instructor</div>
                          <div className="text-sm font-black text-orange-800">Dr. Vikram Mehta</div>
                       </div>
                    </div>
                    <div className="flex items-center gap-6">
                       <div className="text-center">
                          <div className="text-[10px] font-black text-slate-500 uppercase mb-1">Participants</div>
                          <div className="text-sm font-black text-orange-800 flex items-center gap-2">
                             <Users size={14} className="text-orange-800" /> 1,240+
                          </div>
                       </div>
                       <div className="text-center">
                  ...
                  <div className="flex gap-4 pt-4">
                    <button 
                      ...
                      className="px-10 py-5 bg-brand-orange text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-orange-600 transition-all shadow-xl shadow-orange-500/20 active:scale-95 flex items-center gap-3"
                    >
                       <Video size={18} /> {isInteractiveMode ? 'Exit Interactive Mode' : 'Enter Interactive Room'}
                    </button>
                    <button className="px-10 py-5 bg-white border border-slate-200 text-orange-800 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all hover:bg-slate-50">
                       View Syllabus
                    </button>
                  </div>
               </div>
                      ...
                      <div className="bg-white border-2 border-slate-100 rounded-xl p-8 shadow-inner min-h-[400px] relative overflow-hidden flex flex-col">
                         <div className="absolute top-4 left-4 flex gap-2">
                            {[MousePointer2, PenTool, Eraser, Layers].map((Icon, i) => (
                               <button key={i} className="p-3 bg-slate-50 text-slate-400 hover:bg-brand-orange hover:text-white rounded-xl transition-all">
                                  <Icon size={18} />
                               </button>
                            ))}
                         </div>
                         <div className="absolute top-4 right-4 flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-500 rounded-xl text-[8px] font-black uppercase tracking-widest">
                            <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /> Shared Canvas Active
                         </div>
                         ...
                         <div className="flex justify-center gap-4 pt-6 mt-auto border-t border-slate-50">
                            <button className="px-6 py-3 bg-slate-900 text-white rounded-xl text-[8px] font-black uppercase tracking-widest hover:bg-brand-orange transition-all">Download Frame</button>
                      ...
                      <div className="space-y-6">
                         <div className="p-6 bg-slate-900 rounded-xl text-white">
                            <div className="flex items-center gap-2 mb-6">
                               <Columns size={16} className="text-brand-orange" />
                            ...
                            <button className="w-full mt-6 py-4 bg-brand-orange text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg shadow-orange-500/20">Assign Peer Groups</button>
                         </div>

                         {/* Real-time Poll CTA */}
                         <div className="p-6 bg-white border border-slate-100 rounded-xl group cursor-pointer hover:border-brand-orange/20 transition-all" onClick={() => setShowPoll(!showPoll)}>
                        ...
                        >
                           ...
                           <div className="space-y-3">
                              {['Very Confident', 'Need more practice', 'Not understood'].map((opt, i) => (
                                 <button key={i} className="w-full p-4 bg-slate-50 hover:bg-brand-orange hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all text-left">
                                    {opt}
                                 </button>
                              ))}
                           </div>
           ...
           <div className="space-y-8">
              ...
              <div className="grid gap-4">
                 {schedule.map((item) => {
                   ...
                   return (
                   <div key={item.id} className="group p-8 rounded-xl bg-white border border-slate-100 hover:border-brand-orange/20 hover:shadow-2xl hover:shadow-slate-200/50 transition-all flex flex-col lg:flex-row items-center gap-10 cursor-pointer" onClick={() => {
                     setSelectedDate(eventDate);
                     setShowEventModal(true);
                   }}>
                      <div className="w-16 h-16 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center shrink-0 group-hover:bg-brand-orange group-hover:text-white transition-all shadow-sm">
                         <Clock size={24} />
                      </div>
                      ...
                      <div className="flex items-center gap-6">
                         ...
                         <button 
                           ...
                           className="px-8 py-4 bg-slate-50 text-slate-900 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-brand-orange hover:text-white transition-all shadow-sm flex items-center gap-2"
                           onClick={(e) => {
                             e.stopPropagation();
        ...
        <div className="space-y-8">
              ...
              <div className="flex items-center justify-between mb-8">
                 ...
                 <div className="flex items-center gap-2">
                    <button 
                      ...
                      className="w-10 h-10 rounded-xl bg-slate-50 text-slate-400 hover:bg-brand-orange hover:text-white flex items-center justify-center transition-all shadow-sm"
                    >
                      ‹
                    </button>
                    <button 
                      ...
                      title="Today"
                      className="px-4 py-2 bg-brand-orange text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg"
                    >
                      Today
                    </button>
                    <button 
          ...
          <div className='p-8 rounded-xl bg-amber-100 text-black relative  overflow-hidden group'>
                 ...
                 {[
                   ...
                   { icon: <Play className="w-4 h-4" />, label: 'Archive Hub' },
                    ...
                    <button key={i} className="w-full p-4 bg-white/5 hover:bg-white/10 rounded-xl flex items-center gap-3 transition-all group">
                       <div className="text-brand-orange group-hover:scale-110 transition-transform">{tool.icon}</div>
           ...
           {/* Quick Support */}
           <div className="p-8 rounded-xl bg-gradient-to-br from-amber-50 to-white border border-blue-100 shadow-sm text-center">
              <Mail className="w-10 h-10 mx-auto mb-4 text-orange-500 opacity-50" />
              ...
              <button className="w-full py-4 bg-orange-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-orange-600 transition-all shadow-xl shadow-orange-500/20">
                 Contact Helpdesk
              </button>
           </div>
        </div>
