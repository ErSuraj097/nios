    ...
    >
      <div className="grid lg:grid-cols-4 gap-8 mb-8">
        {[
          ...
          { label: 'Open Tickets', value: '142', icon: Headphones, color: 'text-brand-orange', alert: true },
          { label: 'Cyber Alerts', value: '12', icon: ShieldAlert, color: 'text-red-500', alert: true },
        ].map((stat, i) => (
           <div key={i} className="p-6 rounded-xl bg-white border border-slate-100 shadow-sm relative overflow-hidden flex items-center justify-between">
              ...
              <div className={`p-4 rounded-xl ${stat.alert ? 'bg-red-50 animate-pulse' : 'bg-slate-50'}`}>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col h-[600px]">
        ...
        <div className="border-b border-slate-100 p-6 flex items-center justify-between">
           <div className="flex bg-slate-100 p-1 rounded-xl">
              <button onClick={() => setActiveTab('tickets')} className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'tickets' ? 'bg-white text-brand-orange shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>
                 Ticketing SLA
              </button>
           ...
           {activeTab === 'tickets' && (
              <div className="uppercase text-[10px] font-black tracking-widest text-slate-400">
                 SLA Matrix: <span className="text-red-500">High (&lt;1h)</span> · <span className="text-amber-500">Medium (&lt;4h)</span> · <span className="text-blue-500">Low (&lt;24h)</span>
              </div>
           )}
        </div>
                       ...
                       <tr key={tkt.id} className="hover:bg-slate-50/50 cursor-pointer">
                          ...
                          <td className="px-6 py-4">
                             <div className="flex items-center gap-2">
                                <span className={`w-2 h-2 rounded-full ${tkt.priority === 'High' ? 'bg-red-500 animate-pulse' : tkt.priority === 'Medium' ? 'bg-amber-500' : 'bg-blue-500'}`} />
                          ...
                          <td className="px-6 py-4 text-right">
                             <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest ${
                                ...
                                tkt.status === 'escalated' ? 'bg-red-100 text-red-600 border border-red-200' : 'bg-slate-100 text-slate-600'
                             }`}>
                                {tkt.status}
                             </span>
                          </td>
              ...
              <div className="p-6 space-y-6">
                 ...
                 <div className="grid gap-4">
                    {mockCyberAlerts.map(alert => (
                       <div key={alert.id} className="p-5 rounded-xl border border-red-100 bg-white hover:border-red-300 transition-all shadow-sm">
                          <div className="flex items-start justify-between mb-4">
                             ...
                             <span className="px-3 py-1 rounded bg-red-100 border border-red-200 text-red-600 text-[10px] font-black uppercase tracking-widest animate-pulse">
                                {alert.severity}
                             </span>
                          </div>
                          ...
                          <div className="flex items-center gap-3 pt-4 border-t border-slate-50">
                             ...
                             <button className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-black uppercase rounded-lg transition-all">Suspend User</button>
                          </div>
                       </div>
                    ))}
                 </div>
