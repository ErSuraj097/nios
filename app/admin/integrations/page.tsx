    ...
    >
      <div className="grid lg:grid-cols-2 gap-8 mb-8">
              ...
              <div key={api.id} className="p-5 rounded-xl bg-white border border-slate-100 shadow-sm flex items-center justify-between group hover:border-slate-300 transition-all">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                    api.status === 'connected' ? 'bg-emerald-50 text-emerald-500' :
                ...
                <div className="text-right">
                  ...
                  {api.status === 'error' && <span className="inline-flex text-red-600 font-bold text-xs"><ShieldAlert size={14} className="mr-1" /> Failed</span>}
        ...
        <div className="space-y-6">
          ...
          <div className="p-8 rounded-xl bg-gradient-to-br from-orange-500 to-red-600 text-white relative overflow-hidden shadow-lg border border-orange-400/50">
             <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl -mt-10 -mr-10" />
             <div className="flex items-start justify-between mb-8 relative z-10">
                <div>
                   <h4 className="text-sm font-black uppercase tracking-widest text-orange-100 mb-1">QR Verification System</h4>
                   <div className="text-3xl font-black">2.4M+</div>
                   <div className="text-xs font-semibold text-orange-200">Certificates Generated</div>
                </div>
             ...
             <div className="grid grid-cols-2 gap-4 relative z-10">
                <button className="flex items-center justify-center gap-2 p-4 bg-white/10 hover:bg-white/20 transition-colors rounded-xl border border-white/20 text-sm font-bold backdrop-blur-md">
                   <ImageIcon size={18} /> Update Branding
                </button>
                <button className="flex items-center justify-center gap-2 p-4 bg-white/10 hover:bg-white/20 transition-colors rounded-xl border border-white/20 text-sm font-bold backdrop-blur-md">
