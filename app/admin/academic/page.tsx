    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
         <div className="flex bg-slate-100 p-1 rounded-xl">
            ...
            <button onClick={() => setActiveTab('content')} className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'content' ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>
         ...
         <div className="relative">
            ...
            <input 
               ...
               className="pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-brand-orange w-64"
               value={searchTerm}
                     ...
                     <tr key={teacher.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                           <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest ${
                              ...
                              teacher.status === 'suspended' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-600'
                           }`}>
                        ...
                        <td className="px-6 py-4 text-right space-x-2">
                           ...
                           {teacher.status === 'active' && (
                              <button className="px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all">Suspend</button>
                           )}
                           ...
                           <td className="px-6 py-4 text-right space-x-2">
                              {course.status === 'pending_review' && (
                                 <>
                                    ...
                                    <button className="px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all"><XCircle size={12} className="inline mr-1"/> Reject</button>
                                 </>
                              )}
