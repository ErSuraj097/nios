'use client';
  ...
  export default function AITutorPage() {
    ...
    const suggestions = quickSuggestions.map((title, i) => ({
      ...
      color: ['text-amber-500 bg-amber-50', 'text-purple-500 bg-purple-50', 'text-emerald-500 bg-emerald-50'][i % 3]
    }));
      ...
      >
        <div className="grid lg:grid-cols-[1fr_320px] gap-8 ">
          ...
          <div className="flex flex-col min-h-[80vh] max-h-[85vh] bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden animate-fade-in ">
            ...
            <div className="p-6 border-b border-slate-50 bg-gradient-to-r from-amber-50 to-orange-50  border-b border-slate-200 backdrop-blur-md z-10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-brand-orange/10 flex items-center justify-center text-brand-orange ring-4 ring-orange-50">
            <Sparkles size={20} />
                </div>
                <div>
              ...
              <div className="max-w-xl mx-auto space-y-8">
                <AnimatePresence>
                  {messages.map((message) => (
                    ...
                    >
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${message.role === 'ai' ? 'bg-brand-orange/10 text-brand-orange' : 'bg-slate-900 text-white'}`}>
                ...
                {isLoading && (
                  ...
                  >
                    <div className="w-8 h-8 rounded-xl bg-brand-orange/10 flex items-center justify-center text-brand-orange shrink-0 animate-pulse">
                      <Sparkles size={16} />
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 animate-fade-in">
                    {suggestions.slice(0, 3).map((s, i) => (
                      <button 
                        ...
                        className="p-6 text-left rounded-xl bg-white border border-slate-100 hover:border-brand-orange/30 hover:shadow-xl transition-all group cursor-pointer"
                      >
            ...
            className="w-full bg-slate-50 border border-slate-100 rounded-xl py-5 px-8 pr-32 text-sm focus:outline-none focus:ring-4 focus:ring-brand-orange/5 focus:border-brand-orange/20 transition-all resize-none shadow-sm min-h-[64px]"
            rows={1}
                />
            ...
            <button className="p-2 text-slate-400 hover:text-brand-orange transition-colors" title="Attach file">
              <Paperclip size={20} />
            </button>
            <button 
              onClick={handleSend}
              disabled={!input.trim() || isLoading}
              className={`p-2.5 rounded-xl bg-brand-orange text-white shadow-lg shadow-orange-500/20 active:scale-95 transition-all ${input.trim() && !isLoading ? 'opacity-100 hover:shadow-orange-500/40' : 'opacity-50 grayscale cursor-not-allowed'}`}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="pt-6 border-t border-white/10">
                      <div className="text-[8px] font-black text-brand-orange uppercase tracking-[0.2em] mb-4">Quick Actions</div>
                      <button className="w-full p-4 bg-brand-orange text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg shadow-orange-500/20 flex items-center justify-center gap-2 group">
                    ...
                    <div className="pt-4 mt-4 border-t border-white/10">
                      <button className="flex items-center justify-between w-full p-4 bg-white/5 rounded-xl hover:bg-brand-orange transition-all group">
