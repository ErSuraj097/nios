    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <button
          ...
          className={` p-2 border-r border-b border-slate-50 hover:bg-orange-50 transition-all group cursor-pointer relative flex flex-col ${
            isToday ? 'bg-orange-100 ring-2 ring-orange-300' : dayEvents.length ? 'bg-blue-50 hover:bg-blue-100' : ''
          }`}
        >
          <span className={`text-xs font-black text-center ${isToday ? 'text-orange-600' : dayEvents.length ? 'text-blue-600' : 'text-slate-500'}`}>
            {day}
          </span>
          {dayEvents.length > 0 && (
            <div className="mt-auto w-full h-1.5 bg-gradient-to-r from-blue-400 to-orange-400 rounded-t-full mt-1 shadow-sm" />
          )}
        </button>
      );
    }
                  ...
                  <div key={event.id} className="p-6 border rounded-xl hover:shadow-md transition-all">
                    <div className="flex items-start gap-4 mb-4">
                      <div className={`p-3 rounded-xl flex-shrink-0 ${
                        ...
                        event.type === 'Exam' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'
                      }`}>
                        <Clock size={20} />
                      </div>
        ...
        {showAddModal && (
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                <select className="w-full p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-orange">
                  <option>Class</option>
                  <option>Exam</option>
                  <option>Submission</option>
                </select>
                <input className="w-full p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-orange" placeholder="Title" />
                <input type="time" className="w-full p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-orange" />
                <select className="w-full p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-orange">
                  <option>Zoom</option>
        ...
        <div className="space-y-8 flex flex-col overflow-hidden">
           ...
           <div className="p-8 rounded-xl  bg-slate-900 text-white shadow-xl relative overflow-hidden group">
              ...
              <div className="space-y-3">
                 <button className="w-full py-4 bg-brand-orange hover:bg-orange-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest transition-all shadow-lg shadow-orange-500/20 active:scale-95 flex items-center justify-center gap-2">
           ...
           <div className="p-8 border-b border-slate-50 flex items-center justify-between">
                 <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2">
                       <button 
                         ...
                         className="p-3 hover:bg-slate-50 rounded-xl transition-all text-slate-400 hover:text-brand-orange shadow-sm"
                       >
                         <ChevronLeft size={20} />
                       </button>
                    ...
                    {['Month', 'Week', 'Day'].map((v) => (
                      <button 
                        ...
                        className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm ${
                          view === v.toLowerCase() ? 'bg-brand-orange text-white shadow-lg shadow-orange-500/20' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-900'
                        }`}
              ...
              <div className="flex items-center gap-4">
                 ...
                 <button 
                   ...
                   title="Go to Today"
                   className="px-2 py-2 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-brand-orange transition-all shadow-lg"
                 >
                   Today
                 </button>
                 <input
                   ...
                   className="px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium placeholder-slate-400 focus:ring-2 focus:ring-brand-orange focus:border-transparent w-48 transition-all shadow-sm"
                 />
              </div>
           </div>
                ...
                <div className="p-8 space-y-8">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {eventsOnDate(selectedDate || currentDate).map(e => (
                      <div key={e.id} className="p-6 bg-gradient-to-r from-blue-50 to-orange-50 rounded-xl shadow-lg hover:shadow-xl transition-all">
                        <div className="flex items-center gap-3 mb-4">
                          <div className={`p-2 rounded-xl ${
                            e.type === 'Class' ? 'bg-blue-200' : e.type === 'Exam' ? 'bg-red-200' : 'bg-orange-200'
                          }`}>
                            <Clock size={16} />
                          </div>
                          <div>
                        ...
                        <button className="w-full py-3 bg-white border-2 border-slate-200 rounded-xl font-black text-sm uppercase tracking-widest hover:border-brand-orange hover:bg-brand-orange/5 transition-all">
                          Join/View
                        </button>
                      </div>
                    ))}
