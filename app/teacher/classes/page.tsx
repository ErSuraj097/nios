      ...
      <div className="space-y-10 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center justify-between">
           <div className="flex items-center gap-6">
              <button className="px-8 py-5 bg-slate-900 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200 flex items-center gap-3">
           ...
           <button className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm text-slate-400 hover:text-slate-900 transition-all">
             <Settings size={18} />
           </button>
        </div>
        ...
        <div className="space-y-6">
           ...
           <div className="grid gap-6">
              {classes.map((cls) => (
                <div key={cls.id} className="group p-8 rounded-[3rem] bg-white border border-slate-100 hover:border-brand-orange/20 hover:shadow-2xl hover:shadow-slate-200/50 transition-all flex flex-col lg:flex-row items-center gap-10">
                   <div className="w-20 h-20 rounded-[2rem] bg-orange-50 text-brand-orange flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform shadow-sm">
                      <Video size={32} />
                   </div>
                   
                   <div className="flex-1 text-center lg:text-left min-w-0">
                      <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">{cls.stream} · {cls.type}</div>
                      <h4 className="text-xl font-black text-slate-900 group-hover:text-brand-orange transition-colors mb-4">{cls.title}</h4>
                   ...
                   <div className="flex items-center gap-4 py-8 px-10 border-x border-slate-50 hidden lg:flex">
                      <div className="text-center">
                         ...
                         <div className={`text-[10px] font-black uppercase tracking-widest ${
                           cls.status === 'Upcoming' ? 'text-amber-500 underline decoration-2' : 'text-emerald-500'
                         }`}>{cls.status}</div>
                      </div>
                   </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
           <div className="p-10 rounded-[3rem] bg-white border border-slate-100 shadow-sm relative overflow-hidden group">
              ...
              <div className="flex items-center gap-3 mb-8 text-brand-orange">
                 <BarChart3 size={24} />
              ...
              <button className="text-[10px] font-black text-slate-900 uppercase tracking-widest border-b-2 border-brand-orange pb-1 hover:text-brand-orange transition-colors">Generate Reports</button>
           </div>
           ...
           <div className="p-10 rounded-[3rem] bg-slate-900 text-white relative overflow-hidden group">
              ...
              <div className="flex items-center gap-3 mb-8">
                 <Monitor size={24} className="text-brand-orange" />
              ...
              <button className="px-8 py-4 bg-white/10 hover:bg-white/20 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all">Upload Materials</button>
           </div>
        </div>
      </div>
    </DashboardLayout>
